...
function initContex() {
  service.get(`/user/selectLimit?page=${operationValue.page}&identity=3&search=${operationValue.search}`).then(
      (res) => {
...
<template>
  <div style="position: relative;height: 102vh">
    <t-dialog
        placement="center"
        :header="operationValue.isUpdate?'编辑管理员信息':'新增管理员'"
  ...
  defaultAddress: [{required: true, message: '地址不能为空'}]
        ...
        <t-form-item style="width: 96%;margin: 8px 0" label="密码" name="password">
          <t-input v-if="!operationValue.isUpdate" v-model="operationValue.pojo.password"
                   placeholder="请输入密码"></t-input>
          <t-alert STYLE="width: 100%;height: 40px;padding-top: 10px" v-else theme="warning"
                   message="你没有权限修改密码"/>
