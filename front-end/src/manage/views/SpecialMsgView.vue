...
function initContex() {
  service.get(`/specia/selectLimit?page=${operationValue.page}&search=${operationValue.search}`).then(
      (res) => {
...
async function beforeSubmit() {
  if (typeof await form.value.validate() === 'boolean') {
    service.post(`/specia/${operationValue.isUpdate?'update':'insert'}`, operationValue.pojo).then( (res) => {
      console.log(11121)
      console.log(res)
      initContex()
      if (res.data>0){
...
function deletes(id) {
  service.post(`/specia/deletes?ids=`+id.toString() ).then((res) => {
    console.log(res)
    initContex()
    if (res.data>0){
...
<template>
  <div style="position: relative;height: 102vh">
    <t-dialog
        placement="center"
        :header="operationValue.isUpdate?'编辑专业':'新增专业'"
  ...
  name: [{required: true, message: '内容不能为空'}],
 }" :data="operationValue.pojo" :colon="true">
        <t-form-item style="width: 96%;margin: 8px 0" label="专业名称" name="name">
      ...
      <t-table
          ...
          :columns="[
           { colKey: 'row-select', type: 'multiple' ,width: 60,},
            { colKey: 'id', title: 'ID'},
            { colKey: 'name', title: '专业名称',ellipsis: true,},
