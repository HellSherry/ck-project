...
function initContex() {
  service.get(`/leaverequest/selectAll?page=${operationValue.page}&size=8&names=${operationValue.search}`).then(
      (res) => {
...
async function beforeSubmit() {
  ...
  if (typeof await form.value.validate() === 'boolean') {
    service.post(`/leaverequest/${operationValue.isUpdate ? 'update' : 'insert'}`, operationValue.pojo).then((res) => {
      console.log(res)
      initContex()
...
function deletes(id) {
  service.post(`/leaverequest/delete?ids=` + id.toString()).then((res) => {
    console.log(res)
    initContex()
    if (res.data > 0) {
...
<template>
  <div style="position: relative;height: 102vh">
    <t-dialog
        placement="center"
        :header="operationValue.isUpdate?'编辑假条信息':'新增假条'"
  ...
  eeason: [{required: true, message: '请假理由不能为空'}],
  status: [{required: true, message: '状态不能为空'}],
  batchdummy: [{required: true, message: '批假人不能为空'}]
...
<!--        </t-form-item>-->
        <t-form-item style="width: 96%;margin: 8px 0" label="学员姓名" name="name">
          <t-input v-model="operationValue.pojo.name" placeholder="请输入学员姓名"></t-input>
        </t-form-item>
        <t-form-item style="width: 96%;margin: 8px 0" label="开始时间" name="startdate">
          <t-date-picker  v-model="operationValue.pojo.startdate"/>
<!--          <t-input placeholder="请输入开始时间"></t-input>-->
        </t-form-item>
        <t-form-item style="width: 96%;margin: 8px 0" label="结束时间" name="enddate">
          <t-date-picker  v-model="operationValue.pojo.enddate"/>
        </t-form-item>
        <t-form-item style="width: 96%;margin: 8px 0" label="请假理由" name="eeason">
          <t-input v-model="operationValue.pojo.eeason" placeholder="请输入请假理由"></t-input>
        </t-form-item>

        <t-form-item style="width: 96%;margin: 8px 0" label="状态" name="status">
          <!-- 方式一：使用 options 输出下拉选项。优先级高于 t-option-->
          <t-select
              v-model="operationValue.pojo.status"
              :options="operationValue.options1"
              placeholder="请选择审批状态"
              clearable
              @focus="onFocus"
              @blur="onBlur"
        ...
        <t-form-item style="width: 96%;margin: 8px 0" label="批假人" name="batchdummy">
          <t-input v-model="operationValue.pojo.batchdummy" placeholder="请输入批假人"></t-input>
        </t-form-item>
      ...
      <t-table
          ...
          :columns="[
           { colKey: 'row-select', type: 'multiple' ,width: 60,},
            { colKey: 'id', title: 'ID'},
            { colKey: 'name', title: '学员姓名',ellipsis: true,},
            // { colKey: 'paperDescription', title: 'paperDescription'},
            { colKey: 'startdate', title: '开始时间'},
             { colKey: 'enddate', title: '结束时间'},
            // { colKey: 'userImg', title: '用户头像'},
            { colKey: 'eeason', title: '请假理由'},
            { colKey: 'status', title: '状态'},
            { colKey: 'batchdummy', title: '批假人'},
            {
    title: '操作',
    colKey: 'operation',
      }
