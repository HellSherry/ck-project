...
function bingAi() {
  ...
  //       // 总是会执行
  //     });
  service.get("/ai/qwen?ask=" + nowQu.value)
      .then(async result => {
        messages.value.push({msg: JSON.parse(result.data).output.text, type: 'AI', time: new Date().toLocaleString()})
        console.log(result)
        localStorage.setItem("messages", JSON.stringify(messages.value))
      }).catch(() => {
    messages.value.push({msg: '我不会回答你，我太弱了', type: 'AI', time: new Date().toLocaleString()})
    localStorage.setItem("messages", JSON.stringify(messages.value))
  })
      .finally(() => {
        nowQu.value = '';
...
<template>
  <!--  <nut-watermark full-page="false"  :z-index="1" content="nut-watermark"></nut-watermark>-->
  ...
  <div style="width: 80%;margin: 30px 10%">
    ...
    <t-space style="width: 100%;min-height: 80vh;" direction="vertical">
      ...
      <div :id="'aimsg'+index" v-for="(item,index) in messages">
        ...
        <t-avatar :style="item.type=='AI'?'':'float: right'  " @click="visible=!visible">{{ item.type }}</t-avatar>
        <ToHtml
            :style="'vertical-align: top;max-width: 80%;margin-top: 2px;display: inline-block;margin-left:6px;margin-right:6px;padding: 10px;border-radius: 8px;background-color: var(--td-bg-color-container); width:fit-content;'+(item.type=='AI'?'':'float: right;')"
  ...
  <nut-sticky style="width: 100%;margin-top: 20px ; " bottom="0" position="bottom">

    <div style="height: 10vh;width: 100%;background-color: var(--td-bg-color-page);margin: 0 auto">
