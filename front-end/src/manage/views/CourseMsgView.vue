...
const special = ref([])
service.get('/specia/getAll').then((res) => {
  special.value = res.data;
})


function initContex() {
  service.get(`/questionCourse/selectLimit?page=${operationValue.page}&search=${operationValue.search}`).then(
      (res) => {
...
async function beforeSubmit() {
  if (typeof await form.value.validate() === 'boolean') {
    service.post(`/questionCourse/${operationValue.isUpdate ? 'update' : 'insert'}`, operationValue.pojo).then((res) => {
      initContex()
      if (res.data > 0) {
...
function deletes(id) {
  service.post(`/questionCourse/deletes?ids=` + id.toString()).then((res) => {
    console.log(res)
    initContex()
    if (res.data > 0) {
...
function findSpecial(speciaid) {
  let find = special.value.find(e => e.id === speciaid);
  if(find!=null){
    return find.name
  }
 }
...
<template>
  <div style="position: relative;height: 102vh">
    <t-dialog
        placement="center"
        :header="operationValue.isUpdate?'编辑课程信息':'新增课程'"
  ...
  speciaid: [{required: true, message: '专业不能为空'}]
}" :data="operationValue.pojo" :colon="true">
        <t-form-item style="width: 96%;margin: 8px 0" label="课程名称" name="name">
          <t-input v-model="operationValue.pojo.name" placeholder="请输入内容"></t-input>
        </t-form-item>
        <t-form-item style="width: 96%;margin: 8px 0" label="详情" name="coursedescription">
          <t-input v-model="operationValue.pojo.coursedescription" placeholder="请输入内容"></t-input>
        </t-form-item>
        <t-form-item style="width: 96%;margin: 8px 0" label="专业" name="speciaid">
          <t-radio-group v-model="operationValue.pojo.speciaid">
            <t-radio v-for="{id,name} in special" :value="id">{{ name }}</t-radio>
          </t-radio-group>
        </t-form-item>
      ...
      <t-table
          ...
          :columns="[
           { colKey: 'row-select', type: 'multiple' ,width: 60,},
            { colKey: 'id', title: 'ID'},
            { colKey: 'name', title: '课程名',ellipsis: true,},
            { colKey: 'coursedescription', title: '详情'},
            { colKey: 'speciaid', title: '专业'},
             {
    title: '操作',
    colKey: 'operation',
      }
      ...
      >
        ...
        <template #speciaid="{ row }">
          {{  findSpecial(row.speciaid) }}
        </template>
