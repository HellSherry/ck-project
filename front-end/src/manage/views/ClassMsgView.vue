...
async function beforeSubmit() {
  if (typeof await form.value.validate() === 'boolean') {
    service.post(`/classInfo/${operationValue.isUpdate?'update':'insert'}`, operationValue.pojo).then( (res) => {
      initContex()
      if (res.data>0){
...
function deletes(id) {
  service.post(`/classInfo/deletes?ids=`+id.toString() ).then((res) => {
    console.log(res)
    initContex()
    if (res.data>0){
...
<template>
  <div style="position: relative;height: 102vh">
    <t-dialog
        placement="center"
        :header="operationValue.isUpdate?'编辑班级信息':'新增班级'"
  ...
  speciaid: [{required: true, message: '内容不能为空'}]
}" :data="operationValue.pojo" :colon="true">
        <t-form-item style="width: 96%;margin: 8px 0" label="班级名称" name="name">
      ...
      <t-table
          ...
          :columns="[
           { colKey: 'row-select', type: 'multiple' ,width: 60,},
            { colKey: 'id', title: 'ID'},
            { colKey: 'name', title: '班级名',ellipsis: true,},
