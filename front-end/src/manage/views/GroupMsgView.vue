...
const course = ref([])
service.get('/questionCourse/getAll').then((res) => {
  course.value = res.data;
})


function initContex() {
  service.get(`/questionGroup/selectLimit?page=${operationValue.page}&search=${operationValue.search}`).then(
      (res) => {
...
async function beforeSubmit() {
  if (typeof await form.value.validate() === 'boolean') {
    service.post(`/questionGroup/${operationValue.isUpdate ? 'update' : 'insert'}`, operationValue.pojo).then((res) => {
      initContex()
      if (res.data > 0) {
...
function deletes(id) {
  service.post(`/questionGroup/deletes?ids=` + id.toString()).then((res) => {
    console.log(res)
    initContex()
    if (res.data > 0) {
...
function findCourse(speciaid) {
  let find = course.value.find(e => e.id === speciaid);
  if(find!=null){
    return find.name
  }
 }
...
<template>
  <div style="position: relative;height: 102vh">
    <t-dialog
        placement="center"
        :header="operationValue.isUpdate?'编辑章节信息':'新增章节'"
  ...
  name: [{required: true, message: '章节名称不能为空'}],
  courseid: [{required: true, message: '专业不能为空'}]
}" :data="operationValue.pojo" :colon="true">
        <t-form-item style="width: 96%;margin: 8px 0" label="章节名称" name="name">
          <t-input v-model="operationValue.pojo.name" placeholder="请输入内容"></t-input>
        </t-form-item>
  
        <t-form-item style="width: 96%;margin: 8px 0" label="课程" name="courseid">
          <t-radio-group v-model="operationValue.pojo.courseid">
            <t-radio v-for="{id,name} in course" :value="id">{{ name }}</t-radio>
          </t-radio-group>
        </t-form-item>
      ...
      <t-table
          ...
          :columns="[
           { colKey: 'row-select', type: 'multiple' ,width: 60,},
            { colKey: 'id', title: 'ID'},
            { colKey: 'name', title: '章节名',ellipsis: true,},
            { colKey: 'speciaid', title: '章节'},
             {
    title: '操作',
    colKey: 'operation',
      }
